...
NosTale has two currencies: **gold** (farmable in-game) and **NosDollars / ND** (premium currency, only buyable with real money). The **NosMall** cash shop sells items in ND only. The **NosBazar** is the player auction house where listings settle in gold.

Because NosMall items can only be bought with ND but resold for gold, there's a natural arbitrage market: players who farm gold buy NosMall items second-hand from players who spend cash, paying gold for them. This creates a stable **gold-per-ND exchange rate** that varies by item.

A non-paying player who wants to compete in NosTale's upgrade systems (Specialist Card upgrades, equipment upgrades - both have destructive failures that the NosMall protection scrolls insure against) needs to know the current gold-per-ND rate for every NosMall item, so they know which one to buy second-hand for the best deal. The same applies in reverse for selling: keep your listings competitive without manually re-listing every few minutes.

This bot automates both sides:
- `-monitor` mode polls the cheapest current NosBazar listing for every item you track and shows the live gold-per-ND ratio in a sorted table. The most profitable buys are at the top.
- Default mode keeps your own listings priced 1 gold below the cheapest competitor so you don't have to babysit them.

See `CLAUDE.md` for the full game-context writeup with sources.
